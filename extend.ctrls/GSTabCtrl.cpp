...
void GSTabCtrl::OnPaint(IRenderTarget *pRT)
{
	...
	SOUI::CRect rcItem, rcItemPrev;
	SOUI::CRect rcSplit;
	DWORD dwState;
	SOUI::CRect rcTitle = GetTitleRect();
	...
	if (m_pSkinFrame)
	{
		SOUI::CRect rcPage = GetChildrenLayoutRect();
...
void GSTabCtrl::DrawItem(IRenderTarget *pRT, const SOUI::CRect &rcItem, int iItem, DWORD dwState)
{
...
// 		}
	...
	SOUI::CRect rcIcon(m_ptIcon + rcItem.TopLeft(), SOUI::CSize(0, 0));
	if (m_pSkinIcon)
	{
...
void GSTabCtrl::OnMouseMove(UINT nFlags, SOUI::CPoint point)
{
	SOUI::CRect rcItem;
