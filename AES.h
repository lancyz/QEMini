...
public:
	...
	virtual ~AES();
	//加密整段文字
	void Bm53Cipher(char *input,char *output);	//将加密结果转成16进制返回
	//void Bm64Cipher(char *input,char *output);	//将加密结果编码为base64格式返回
...
private:	
	...
	void CipherToByte(char *input, char *output);	
	void CipherToHex(char *input, char *output);	
