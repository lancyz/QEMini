...
void AES::CipherToHex(char *input, char *output)
{
  unsigned char uch_input[1024];
  memset(uch_input, 0, 1024);
...
void AES::CipherToByte(char *input, char *output)
{
	unsigned char uch_input[1024];
	memset(uch_input, 0, 1024);
	strToUChar(input, uch_input);
	...
	memcpy(output, uch_input, strlen((char*)uch_input));
	//ucharToHex(uch_input,output);
}


//void AES::Bm64Cipher(char *input, char *output)
//{
...
//	std::string strOut;
//	CBase64 base64;
//	base64.Encode((unsigned char*)output, strlen((char*)output), strOut);
//
//	memcpy(output, strOut.c_str(), strOut.length());
//
//	free(newAlock);
//}
